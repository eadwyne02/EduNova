        ...
        <div className='flex justify-between'>
          ...
          <nav className='hidden md:flex items-center gap-8'>
            {navLinks.map((navLink, index) => {
              ...
              return (
                <Link key={index} to={navLink.path} className={`text-lg transition-colors duration-200 ${isActive ? 'text-[#1E3A8A] font-semibold' : 'text-[#1E3A8A]/75 hover:text-[#1E3A8A]'}`}>
                  {navLink.label}
                </Link>
              )
            })}
          ...
          <div className='hidden md:flex items-center gap-6'>
            <Link to='/portal' className='bg-[#1E3A8A] text-md font-medium text-white/90 px-3 py-2.5 rounded-sm hover:-translate-y-[2px] hover:shadow-sm transition-all duration-300'>
              Portal login
            </Link>
            <Link to='/apply' className='bg-[#B8901F] text-[#1E3A8A] text-md font-semibold px-3 py-2.5 rounded-sm transition-all duration-300 hover:-translate-y-[2px] hover:shadow-sm'>
              Apply now
            </Link>
          </div>
